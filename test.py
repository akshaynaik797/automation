...
class FillPortalData:
    ...
    def __init__(self, mss_no):
        ...
        if r1.status_code == 200:
            self.data = r1.json()
            self.data['0']['PortalUser'] = 'JITENDRA MAX SS'
            self.data['0']['PortalPass'] = 'Maxppg@1234'
            self.data['0']['preauthNo'] = 'DEL-0121-PA-0000982'
            fields = (
        ...
        for i in records:
            try:
                ...
                if i['is_input'] == 'I':
                    ...
                    fill_input(value, path_type, path_value)
                    print(i['step'], i['value'])
                if i['is_input'] == 'B' or i['is_input'] == 'ICON':
                    path_type, path_value = i['path_type'], i['path_value']
                    press_button(path_type, path_value)
                    print(i['step'], i['value'])
                if i['is_input'] == 'F':
                ...
                if i['is_input'] == 'LINK':
                    path_type, path_value = i['path_type'], i['path_value']
                    attribute = get_attribute(path_type, path_value, 'title')
                    if 'Shortfall' not in attribute:
                        press_button(path_type, path_value)
                    else:
                        driver.get(f'https://tips.vidalhealthtpa.com/hospitallogin/hospital/preauth/shortfallletterview.html?shortfallNo={self.data["0"]["preauthNo"]}-S01')
                        list_of_files = glob.glob(attachments_folder + '/*')  # * means all if need specific format then *.csv
                        latest_file = max(list_of_files, key=os.path.getctime)
                        # filename, ext = os.path.splitext(latest_file)
                        # filename = self.data["0"]["preauthNo"] + ext
                        # os.rename(latest_file, os.path.join(attachments_folder, filename))
                    print(i['step'], i['value'])
                    z = 1
            except:
                z = 1
